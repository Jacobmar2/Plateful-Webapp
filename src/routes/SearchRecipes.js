                    ...
                    <Col lg="6">
                        <Form method="post">
                            <InputGroup className="mb-3">
                                <BsForm.Control
                                    placeholder="Enter an recipe name"
                                    defaultValue={term}
                                    name="searchTerm"
                                    size="lg"
                                />
...
export async function action({ request, params }) {
    ...
    if (searchTerm) {
        return redirect(`/SearchRecipes/results/${searchTerm}`);
    } else {
        // Redirect to a default page or handle the case where searchTerm doesn't exist
        return redirect("/");
    }
}